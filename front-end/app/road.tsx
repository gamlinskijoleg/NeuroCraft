...
type ApiDetection = {
    class_name?: string;
    confidence: number;
    // bbox can be returned as [x,y,w,h], {x,y,w,h}, or {x1,y1,x2,y2}
...
export default function RoadDamageScreen() {
    ...
    const performScan = async () => {
        ...
        try {
            ...
            const parsedRows: Array<{ class_name: string; confidence: number }> = (data.detections ?? []).map((d) => ({
    ...
    return (
        <SafeAreaView style={styles.screen}>
            ...
            <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={true}>
                ...
                {/* Start Scan Button */}
                <Pressable
                    style={[styles.scanBtn, !selectedImage || loading ? styles.scanBtnDisabled : null]}
                    onPress={performScan}
                            ...
                            <View style={styles.detectionsList}>
                                {rows.map((row, i) => (
                                    <View key={`${row.class_name}-${i}`} style={styles.detectionItem}>
                                        <Text style={styles.detectionName}>{row.class_name}</Text>
                                        ...
                                        <Text style={styles.confidenceText}>{(row.confidence * 100).toFixed(1)}%</Text>
                                    </View>
                                ))}
                            </View>
                        </View>
